...
from fusion_event_radar import EventRadar
from fusion_action_center import build_action_center, notify_change
...
class Runtime:
    def __init__(self, hub=None, directory=None, notify_research=False, sender=None):
        ...
        self.v8_baseline = V8Baseline(self.directory, self.operations)
        self.event_radar = EventRadar(self.directory, self.operations)
    ...
    def publish(self, now, status, candidates=(), **extra):
        try:
            self.event_radar.step(now, self.fetch, self.preparation, self.latest_quotes,
                                  self.previous_day, allow_refresh=status == 'MARKET_CLOSED',
                                  enabled=self.notify_research)
        except Exception as exc:
            self.event_radar.status = {'status':'EVENT_MODULE_ERROR', 'error':type(exc).__name__,
        ...
        action_center = build_action_center(candidates, self.event_radar.status,
                                            operation_status.get('portfolio') or {})
        action_center['notice_status'] = notify_change(action_center, self.operations, now,
                                                        self.notify_research and status == 'RESEARCH_ONLY')
        ...
        payload['v8_baseline'] = self.v8_baseline.status
        payload['event_radar'] = self.event_radar.status
    ...
    def tick(self, now=None):
        ...
        if day != self.day:
            ...
            self.latest_quotes = {}
        self._calendar(now)
        self.v8_baseline.sync(now, self.notify_research, self.latest_quotes)
        try:
        ...
        # Event discovery supplies candidates only; assess() and its full gates still run.
        selected = list(dict.fromkeys(selected + [code for code in self.event_radar.shortlist() if code in current]))
    ...
    def close(self):
        ...
        self.auction.close()
        self.v8_baseline.close()
        self.event_radar.close()
