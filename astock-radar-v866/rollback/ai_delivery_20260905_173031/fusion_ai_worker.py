...
class Worker:
    ...
    def analyze(self,item,cfg):
        ...
        if cfg.get('notify') is True and fresh(item['first_seen_at'],dt.datetime.now(TZ),180):
            ...
            try:
                import fusion_wecom
                channel=fusion_wecom.channel()
                if not channel.get('configured') or channel.get('fingerprint')!=cfg.get('channel_fingerprint') or channel.get('legacy_fallback'):
                    raise ValueError('Wrong channel')
                message=('新版雷达 | AI证据复核补充\n股票：'+item['code']+'\n原发现时间：'+item['first_seen_at']+'\n复核：'+('证据不足' if review['stance']=='INSUFFICIENT' else '继续观察')+'\n已引用输入证据：'+str(len(review['supported_ids']))+'/'+str(len(item['facts']))+'\n未核验：公告催化、真实可成交性及收益概率。\nAI不能确认建仓，不改变原雷达结论。完整摘要请查看工作台。')
                delivery=fusion_wecom.send_text(message)
                state=delivery.get('status','UNKNOWN') if isinstance(delivery,dict) else 'UNKNOWN'
