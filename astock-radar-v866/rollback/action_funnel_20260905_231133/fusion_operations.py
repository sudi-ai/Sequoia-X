...
class Operations:
    ...
    def portfolio(self, now, quotes, enabled):
        ...
        if not p.exists():
            return {'status':'NO_NEW_VERSION_POSITIONS','positions':0,'alerts':0}
        try:
            rows = json.loads(p.read_text(encoding='utf-8'))
            if not isinstance(rows,list):
                raise ValueError('positions')
        except (OSError,ValueError):
            return {'status':'POSITION_FILE_ERROR','positions':0,'alerts':0}
        count, unknown = 0, 0
        for row in rows:
            ...
            count += 1
        return {'status':'READY' if not unknown else 'PARTIAL_DATA','positions':len(rows),'alerts':count,'unknown':unknown}
    ...
    def run(self, now, worker_status, candidates, calendar, enabled=False, quotes=None):
        ...
        portfolio = self.portfolio(now,quotes or {},enabled) if worker_status == 'RESEARCH_ONLY' else {'status':'PAUSED','positions':None}
        if calendar is True:
            from fusion_market_review import build_brief
