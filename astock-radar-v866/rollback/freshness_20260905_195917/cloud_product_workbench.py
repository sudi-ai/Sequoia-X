...
def render(page, status, code=""):
    ...
    return header+body+'<footer class="pd-footer">云端数据 · 发现记录与交易确认分开显示 · '+link("health","检查更新时间与缺失原因")+'</footer></div>'
...
def install(workbench):
    ...
    def product_page(*args, **kwargs):
        ...
        if mode=="LIVE":
            if page=='portfolio':
                from operator_workspace import operator_panel
                summary='<div class="pd-root">'+operator_panel(load_data())+'</div>'
            ...
            html=html.replace('>DEMO<','>演示<').replace('>LIVE<','>实时<').replace('>PIT<','>历史<')
