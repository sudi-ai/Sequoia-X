...
def render_action_center(data):
    ...
    for row in positions: out.append(f'<p>{text(row.get("ts_code") or "")} · {text(row.get("stage") or "UNKNOWN")} · 现价 {text(row.get("price") if row.get("price") is not None else "未确认")}</p>')
