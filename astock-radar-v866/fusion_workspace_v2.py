...
from fusion_action_workspace import render_action_center
...
def render_payload(payload, now=None):
    ...
    html.append(render_action_center((payload.get('action_center') or {}) if fresh and compatible else {}))
    html.append(render_events(payload.get('event_radar') or {}, now))
    html.append('<h3>新版独立运行与消息台账</h3>')
    html.append(f'<p>V8原始早发现基线：{text(json.dumps(payload.get("v8_baseline", {}),ensure_ascii=False))}</p>')
    html.append('<p>市场摘要：09:35、10:00、13:30、14:30各一次，交易日运行。收盘复盘分列当日研究记录和累计T+1/T+3/T+5结算，不是实盘收益。</p>')
    html.append(f'<p>当天竞价简报（09:26—09:29尝试，缺失不替代）：{text(json.dumps(payload.get("auction", {}),ensure_ascii=False))}</p>')
